...
load ("preprocessed_data_schesaplana.RData")
...
aggregate(1-acc~question+modal+type, data=items_comp_scope, FUN=mean)
...
#m.scope.0.all <- allFit(m.scope.0)

#TODO: Recompute with effect coded contrast
#adjust random effect structure
...
m.scope.exactly.mismatch.0 <- glmer(response_num~modal+(1|item)+(1|id),
                                    family=binomial, data=items_comp_scope_exactly_mismatch,
                                    glmerControl(optimizer = c("bobyqa")))
summary(m.scope.exactly.mismatch.0)

items_comp_scope_exactly_match <- subset(items_comp_scope_exactly, question=="match")

m.scope.exactly.match.0 <- glmer(response_num~modal+(1|item)+(1|id),
...
m.scope.less.1 <- update(m.scope.less.0, .~.-question:modal)
anova(m.scope.less.1, m.scope.less.0)
summary(m.scope.less.0)

m.scope.less.2 <- update(m.scope.less.1, .~.-modal)
anova(m.scope.less.1, m.scope.less.2)
summary(m.scope.less.2)
...
##Bayesian regression weakly informative priors
